...
// #pragma pack(push, 1) 不会影响 fwrite 本身，但会改变结构体的内存布局，从而改变写入文件的字节内容，这对文件格式兼容性至关重要。
//如果你写的是 位图文件头（BITMAPFILEHEADER），必须使用 #pragma pack(push, 1)，因为 BMP 文件格式要求是紧凑排列，不能有多余填充字节。
```
## 智能指针
```c++
#include <iostream>
...
```
## C++ 的数学函数之分类与比较
参考链接 https://mp.weixin.qq.com/s/hpgdqGBkCyrTV7HN-QOdPQ
...
大家都知道，浮点数的计算速度不如定点整数快，那么为什么各类 AI 大模型宁可使用 16 位浮点数，也不用 16 位整数呢？据说，Deepseek 的内核采用的是 8 位浮点数，为什么不用 8 位整数呢？虽然浮点数比定点数计算需要更多的 CPU 周期，但是浮点数的优势是能表示的值域更宽，并且浮点数能表达正负零、无穷大、无穷小和无效值之类的特殊值，在数值计算领域，这些都是常用的值。


浮点数在内存中的存储格式，无非就是指数位占多少比特，尾数占多少比特，符号位怎么安排。以 32 位的 float 为例，按照 IEEE 754 的浮点数格式，它有 1 个符号位，8 个指数位和 23 个尾数位。double 和 long double 也是类似的结构，只是位数不同，


那么浮点数是如何使用这些比特位表示无穷大这样的特殊值呢？IEEE 754 定义了正规浮点数（Normal Floating-Point Number）和次（亚）正规浮点数（Subnormal Floating-Point Number）两个概念，所谓的正规浮点数，就是指数域既不是全 0，也不是全 1 的浮点数，正常的浮点数，比如 2.88，4.2222222，都在这个范围内。次正规浮点数是指指数域全是 0，但尾数域不全是 0 的浮点数，用于表达一些非常接近零的小数。有了这两个定义，浮点数的涵盖范围就完整了，可分为 5 个类别：


正规浮点数：普通的浮点数
次正规浮点数：非常接近零的小数
零（正负）：单独一个类别（指数域和尾数域都是 0）
无穷 INF（正负）：特殊值（指数域全是 1，尾数域全是 0）
无效值（NAN）：特殊值（指数域全是 1，尾数域不全是 0）


那么问题就来了，对于计算过程中出现的结果，我如何知道这个浮点数是否是希望的结果？需要按照 IEEE 754 的标准把指数和尾数都取出来看看？当然不是，C++ 提供了一组判断浮点数类型的函数。

##### 2.1.1 std::isnormal


std::isnormal() 函数就可以用来判断一个浮点数是否是正常的浮点数。那么什么是正常的浮点数呢？按照标准说就是非零、非次正规、非无穷、非无效值的哪些浮点数，看前面的浮点数分类，其实就是第一类的正规浮点数。


##### 2.1.2 std::isinf 和 std::isnan


std::isinf() 函数用来判断一个浮点数是否是正无穷或负无穷，如果是的话就返回 true。std::isnan() 函数用来判断一个浮点数是否是无效值。需要注意的是，无效值只是一个分类，并不是具体的值，看它的分类说明就知道了，尾数域是可以变化的。所以张三计算得到的那个无效值和李四计算得到的无效值可能是不相等的。


一些编译器可以通过设置编译选项允许编译器不处理特殊值，比如 GCC 和 Clang 支持 -ffinite-math 开关，允许编译器假设不存在特殊的浮点数，此时 std::isinf() 和 std::isnan() 的返回值就会总是 false。
...
std::signbit() 函数用来判断一个浮点数是否是负数。它不仅能检查正常的浮点数，也能检查零，无穷大和 NAN 的符号位，所以它可以用来区分正负零和正负无穷大（小）。


##### 2.1.4 std::isfinite

std::isfinite() 函数用来判断一个浮点数是否是个有限值。正规浮点数、次正规浮点数和零是有限值浮点数，无穷和 NAN 就不是。

##### 2.1.5 std::fpclassify

std::fpclassify() 函数用来获取一个浮点数的具体类别。它的返回值不是 bool 类型，而是一个 int，对应以下五个常量值，分别对应 5 种浮点数类型：
...
当然，std::fpclassify() 函数的返回值并不局限于这五个值，标准库的具体实现可以对其进行扩展，这取决于具体的实现。


#### 2.2 浮点数比较

浮点数的比较也是一个一言难尽的话题，我遇到好几次因浮点数比较使用不当导致的 BUG。和整数的规则不一样，它存在一个既不大于也不小于的关系，但是却又不是相等关系。具体来看看 C++ 提供了哪些用于浮点数比较的辅助函数。


##### 2.2.1 std::isgreater 和 std::isgreaterequal

std::isgreater() 和 std::isgreaterequal() 这两个函数用来判断两个浮点数是否是大于或大于等于的关系。那么它们两个和内置的 > 和 >= 运算符有什么区别呢？当 std::isgreater() 和 std::isgreaterequal() 函数遇到无效值（NAN）参数时，它明确不会设置浮点环境中的 FE_INVALID 异常标志位，但是内置的运算符则可能会设置 FE_INVALID 异常标志位。当然，它们和内置比较运算符也有相同的地方，那就是遇到 NAN 的时候，任何比较都返回 false，这就是所谓的 NAN 的“传染性”。


标准库一直都有 std::greater 和 std::greater_equal 函数对象，它们因为实现上使用的是内置的比较运算符，所以同样存在上述区别。

##### 2.2.2 std::isless 和 std::islessequal


std::isless() 和 std::islessequal() 这两个函数判断两个浮点数是否是小于或小于等于的关系。它们两个和内置的比较运算符的不同之处也是对浮点环境中的 FE_INVALID 异常标志位的影响不同，它们明确不影响这个标志位。还有一点和上一节的两个比较函数一样，这两个比较函数遇到 NAN 的时候，都返回 false。


##### 2.2.3 std::islessgreater

当我们做浮点数比较运算的时候，难免会遇到像 NAN 这样的特殊值，在这种情况下，无论你给出的结果是大于或小于都是不合适的。这个函数的意义在于它可以告诉我们两个浮点数是否可比较出一个结果，当然，对于出现 NAN 的时候，它也是不影响浮点环境的标志位的。

前面提到过，在浮点数运算中，NaN 具有"传染性"，也就是说任何涉及 NaN 的比较都返回 false，比如下面的代码：
...
注意对于不等于的判断是 true，因为它是 false 的逻辑否定的结果。对于逻辑关系要求严格的排序算法来说，这种结果是不能接受的。有了 std::islessgreater() 函数，我们就可以在需要严谨的比较结果的场合，先判断两个数是否是可比较的，从而避免与 NAN 值比较产生逻辑错误的结果。以排序为例，对一个包含无效值的数列排序，我们可以这样提供一个安全的浮点数比较器：

```c++
struct SafeLess 
{    
    bool operator()(double a, double b) const {        
        // 如果两个数可以明确比较大小，返回比较结果        
        if (std::islessgreater(a, b)) {            
            return a < b;       
        }        // 否则（涉及NaN），保持相对顺序或特殊处理        
        return std::isnan(a) ? false : true;   
    }
};
```
这个比较器总是假设无效值大于任意正常浮点数，从而通过这个排序，将无效值排在最后面。看看实际的例子：
...
std::sort(data.begin(), data.end(), SafeLess());
std::cout << "排序后: ";   
for (double x : data) {       
    if (std::isnan(x))
        std::cout << "NaN ";       
...
使用安全的比较器，我们得到了预期的结果：1.2，2.1，3.5，4.8，NaN，NaN。如果使用标准库提供的 std::less<double>，我们将得到这样的结果：1.2 3.5 NaN NaN 2.1 4.8。

##### 2.2.4 std::isunordered

std::isunordered() 函数的所用是判断两个数是否是无序的，也就是两个数中的一个和全部都是 NAN。它和 std::islessgreater 的区别就是当 std::isunordered() 函数返回 false 的时候，可以明确知道两个数至少有一个是 NAN。但是当 std::islessgreater() 函数返回 false 的时候，就不一定能确定是否存在 NAN，因为两个数还有可能就是相等关系，此时也是返回 false。
